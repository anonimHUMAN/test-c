...
export default ({ fn, teacher,getAll }) => {
    ...
    const checkInp = async ()=>{
        if(inp1.current.value || inp2.current.value || inp3.current.value  || inp4.current.value  || inp5.current.value  || inp6.current.value ){
            ...
            let user ={
                firstName: inp1.current.value.length>0 ? inp1.current.value : teacher.firstName,
                lastName: inp2.current.value.length>0 ? inp2.current.value : teacher.lastName,
                email: inp3.current.value.length>0 ? inp3.current.value : teacher.email,
                phone: +inp4.current.value.length>0 ? inp4.current.value : teacher.phone,
                password: inp5.current.value.length>0 ? inp5.current.value : teacher.password,
                subject: inp6.current.value.length>0 ? inp6.current.value : teacher.subject,
                role: "teacher"
            }
            let axiosConfig ={
                headers: {
               ...
               let res= await axios.put(`${config.backApi}/admin/teachers/${teacher._id}`, user, axiosConfig)
        }
        data()
        fn()
        getAll()
