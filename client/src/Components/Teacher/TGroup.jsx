...
import Modal1 from "./TGModal/Modal1";
import Modal2 from "./TGModal/Modal2";
import Modal3 from "./TGModal/Modal3";
...
export default () => {
    ...
    async function findOne(id) {
        const DATA = await axios(`${config.backApi}/teacher/group/${id}`, {
            headers: {
    ...
    async function data() {
        ...
        if (new Date(tokenDate) < new Date()) {
            ...
            if (DATA.data.message == "Token wrong!") {
                ...
                setTGroup(DATA.data.data.group)
            }
        }
    }
    const logout = async () => {
    ...
    return (
        <div className={`App ${isDarkMode ? 'dark' : 'light'}`}>
            ...
            <main className="bg-white dark:bg-gray-900 h-screen pt-10">
                ...
                {modal1 && <Modal1 teacher={tGr} getAll={data} fn={chModal1} />}
                {/* <!-- Modal post --> */}
                {modal2 && <Modal2 fn={chModal2} />}
                {/* <!-- Modal profile --> */}
                {modal3 && <Modal3 data={tGr} fn={chModal3} />}
