...
export default ({ fn, getAll }) => {
    ...
    const checkInp = async ()=>{
        if(inp1.current.value === "" || inp2.current.value === "" || inp3.current.value === ""){
            ...
            let group ={
                title: inp1.current.value,
                day: inp2.current.value,
           ...
           let res= await axios.post(`${config.backApi}/teacher/group/?idTeacher=${id}`, group, axiosConfig)
        }
        fn()
        // data()
        getAll()
            ...
            <div className="relative w-10/12 max-h-full">
                ...
                <div className="w-full relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div className="w-full px-6 py-6 lg:px-8 flex flex-row">
                        ...
                        <button onClick={checkInp} type="button" className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Create</button>
                        </form>
                    </div>
                </div>
            </div>
